...
read_csv is different than base R read.csv, because read_csv lets you tell R what TYPE of data (factor, continuous, text, w/e) you want it to be treated as.  To change, it would be something like read_csv("Title", na = c("NA"))
...
2. Which races are present in "The Fellowship of the Ring" (`fell`), but not in 
   ...
   _ok, so, I've read LOTR and I'm pretty sure elf, human, and hobbit characters exist in all 3 books?_
...
Combine all variables from all the tables into one table.  R can automatically pick what to join by so long as the variables have the same names!

```r
all <- left_join(flights, planes) %>% 
  left_join(airlines) %>% 
