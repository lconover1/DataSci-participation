...
You don't have to use the same variable for the whole `case_when()` block! (so, it could be renaming based off lifeExp in one line and gender in the next)
...
gapminder %>% 
  ...
  labs(y = "GDP per capita, $", x = "Life expectancy level, years") +
  theme_bw() +
  scale_x_discrete(drop = FALSE) #required to keep R from dropping the empty "very low" level
```
...
only_five <- c('Austria', 'Paraguay', 'China', 'Ireland', 'Mexico')
  
five_gap <- gapminder %>% 
  filter(country %in% only_five)

nlevels(five_gap$country) #it's the same number! you don't drop levels unless you add a new command
```
...
```{r}
five_gap_dropped <- five_gap %>% 
  droplevels()

nlevels(five_gap_dropped$country)
```
...
gapminder %>%
  ...
  coord_flip() + #puts continent on y-axis instead of x-axis
  theme_bw() +
...
gapminder %>%
  ggplot() +
  geom_bar(aes(fct_reorder(continent, lifeExp, max))) + #reorder continent, use lifeExp to do it, and do it by the maximum lifeExp (as opposed to minimum or something)
  coord_flip() +
  theme_bw() +
...
ggplot(five_gap, aes(x = year, y = lifeExp,
...
```{r}
combo <- c(df1$country, df2$country)
```
You get a buncha integers
...
```{r}
f_combo <- fct_c(df1$country, df2$country)
```
You get a factor with 4 levels!
...
bind_rows(df1, df2)
```
Stuff happens with a lot of warnings.
