...
This goes along with section 6 in the class guide.

Also as a note, you can put two types of plots on top of each other (example being jitter plot and box plot)
...
#alternative: facet_wrap(gapminder$continent)
```
...
("size" is its own arguement under geom_point and shouldn't be placed under aes)
...
## Exercise 5: Life expectancies in Americas (?)
...
gapminder %>% 
  ...
  ggplot(aes(country, lifeExp, alpha = 0.5)) + 
  geom_jitter() +
  geom_boxplot() +
...
mtcars %>% 
  ...
  geom_bar(aes(y = ..prop.., color = "black", fill = transmission), position = "dodge")

mtcars %>% 
  mutate(transmission = if_else(am == 0, "automatic", "manual")) %>% 
  ggplot(aes(x = factor(cyl))) +
  geom_bar(aes(color = "black", fill = transmission), position = "dodge")
...
#I can get proportions on the y axis OR discreet variables on the x axis, but not both.
```
